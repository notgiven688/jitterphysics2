...
namespace FixedMath.Numerics
{
    ...
    /// </summary>
    public struct Fix64Plane : IEquatable<Plane>
    {
        private static readonly Single NormalizeEpsilon = 1.192092896e-07f; // smallest such that 1.0+NormalizeEpsilon != 1.0
        ...
        public static Plane CreateFromVertices(Vector3 point1, Vector3 point2, Vector3 point3)
        {
            if (Vector.IsHardwareAccelerated)
            {
                ...
                // D = - Dot(N, point1)
                Single d = -Vector3.Dot(normal, point1);

                return new Plane(normal, d);
            }
            else
            {
                Single ax = point2.X - point1.X;
                Single ay = point2.Y - point1.Y;
                Single az = point2.Z - point1.Z;

                Single bx = point3.X - point1.X;
                Single by = point3.Y - point1.Y;
                Single bz = point3.Z - point1.Z;

                // N=Cross(a,b)
                Single nx = ay * bz - az * by;
                Single ny = az * bx - ax * bz;
                Single nz = ax * by - ay * bx;

                // Normalize(N)
                Single ls = nx * nx + ny * ny + nz * nz;
        ...
        public static Plane Transform(Plane plane, Matrix4x4 matrix)
        {
            ...
            Single x = plane.Normal.X, y = plane.Normal.Y, z = plane.Normal.Z, w = plane.D;
