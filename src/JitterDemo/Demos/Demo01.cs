...
public class Demo01 : IDemo
{
    ...
    public void Build()
    {
...

        {
            ...
            for (int i = 0; i < numElements; i++)
            {
                ...
                nbody.AddShape(new BoxShape(0.7, 0.1, 4));
                nbody.Position = startPos + new JVector(i * 0.8, 0, 0);

                if (i == 0)
                {
                    var hinge = new HingeJoint(world, world.NullBody, nbody,
                        startPos + new JVector(i * 0.8 - 0.1, 0, 0), JVector.UnitZ);
                }
                else
                {
                ...
                if (i == numElements - 1)
                {
                    var hinge = new HingeJoint(world, nbody, world.NullBody,
                        startPos + new JVector(i * 0.8 + 0.7, 0, 0), JVector.UnitZ);
                }
...

        {
            ...
            JQuaternion rot = JQuaternion.CreateRotationY(Math.PI / 2.0);
    ...
    public void Draw()
    {
        ...
        {
            var h = hinges[i];
            if (h.BallSocket.Impulse.Length() > 0.5)
            {
                h.Remove();
                hinges.RemoveAt(i);
            }
